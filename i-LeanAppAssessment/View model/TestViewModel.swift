...
class TestViewModel {
    ...
    var testList = [Test]()
    var lessonIdToSend = 0
    var testId : Int?
    var lessonId: Int?
    func fetchTestQuestions(key: String, value: Int, completion: @escaping(([Test]?, Error) -> Void)){
        print(123123,lessonIdToSend)
        let networkManager = TestNetworkManager()
        let request = NSMutableURLRequest(url: NSURL(string: "https://app-e-learning-221207163844.azurewebsites.net/user/view/testByLesson?\(key)=\(lessonIdToSend)")! as URL)
...
//                    guard let marks1 = test["marks"] as? Int else {return}
//                    print(marks1)
//                    let testsList = Test(testId: testId1, testName: testName1, duration: duration1, totalQuestions: totalQuestions1, level: level1, marks: marks1)
//                    self.testList.append(testsList)
//                }
//            }
//        }
